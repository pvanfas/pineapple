...
@register.simple_tag
def get_dashboard_settings(request: WSGIRequest) -> dict:
    """
    Get Dashboard settings, update any defaults from the request, and return
    """
...
@register.simple_tag
def get_dashboard_ui_tweaks() -> dict:
    """
    Return Dashboard ui tweaks
    """
...
@register.simple_tag
def dashboard_paginator_number(change_list: ChangeList, i: int) -> SafeText:
    """
...
@register.simple_tag
def dashboard_list_filter(cl: ChangeList, spec: ListFilter) -> SafeText:
    """
...
@register.filter
def has_dashboard_setting(settings: dict[str, Any], key: str) -> bool:
...
def get_sections(
    ...
    if hasattr(admin_form.model_admin, "dashboard_section_order"):
        fieldsets = order_with_respect_to(
            fieldsets,
            admin_form.model_admin.dashboard_section_order,
